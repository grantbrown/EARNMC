prepareKikwitData = function(population=5363500, 
                             beta_SE=c(-1.5,-0.055),
                             beta_RS=c(-100000))
{    
    data(Kikwit95) 
    ...
    list(I_star=matrix(Kikwit95$Count[1:nTpt], ncol=1),
               S0=S0,
               E0=E0,
               I0=I0,
               R0=R0,
               N=matrix(population, ncol = 1, nrow=nTpt),
               X=X,
               Z=Z[1:nTpt,,drop=FALSE],
               X_RS=X_RS,
               gamma_ei=gamma_ei,
               gamma_ir=gamma_ir,
...
kikwitAnalysisKernel = function(cl,fitSeeds, underspecified)
{
    ...
    trueVals = parLapply(cl, paramsList, kikwitModelInstance)
...
kikwitModelInstance = function(params) 
{
    ...
    cat(paste("dim(I_star):", dim(I_star)))
    cat(paste("dim(Z):", dim(simResults$Z)))
...
runKikwitModel = function(fitSeeds=c(812123,12301,5923), underspecified=FALSE)
{                     
    cl = makeCluster(3, outfile = "err.txt")
    print("Cluster Created")
 
    simResults = kikwitAnalysisKernel(cl,fitSeeds,underspecified)
    
    outData = simResults 
    specString = ifelse(underspecified, "underspec", "correct")
    save(outData, file=paste("./simR0_results_", specString, ".tmp", sep=""))
