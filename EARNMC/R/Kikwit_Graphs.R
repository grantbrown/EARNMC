...
makeKikwitGraphPDFs = function(fullSpecResultsFilename=NA,
  ...
  tikz(file = "EA_R0_Comparison_Combined.tex", width=12, height=6, standAlone=TRUE)
  ...
  plot(r01$mean[1:(length(r01$mean) - 1)], ylim = r0.ylim, main = "Traditional ${\\mathcal{R}}_0(t)$: Underspecified Intensity\n Posterior Mode and 90\\% CI",
  ...
  plot(r01$mean[1:(length(r01$mean) - 1)], ylim = r0.ylim, main = "Traditional ${\\mathcal{R}}_0(t)$: Reasonable Intensity\n Posterior Mode and 90\\% CI",
       xlab = "Day", ylab = "Reproductive Number", type = "l", lwd = MainLineWidth)
  abline(h = seq(0,5,0.2), lty = 3,col="lightgrey")
  abline(h = 1.0, col = "blue", lwd = ThresholdWidth, lty = 2)
  lines(1:(length(r01$mean) - 1), r01$LB[1:(length(r01$LB)-1)], lty = 2, lwd = CI_width)
  ...
  plot(r02$mean[1:(length(r02$mean) - 1)], ylim = r0.ylim, main = "$\\mathcal{R}^{({EA})}$: Underspecified Intensity\n Posterior Mode and 90\\% CI",
       xlab = "Day", ylab = "Reproductive Number", type = "l", lwd = MainLineWidth)
  abline(h = seq(0,5,0.2), lty = 3,col="lightgrey")
  abline(h = 1.0, col = "blue", lwd = ThresholdWidth, lty = 2)
  ...
  plot(r02$mean[1:(length(r02$mean) - 1)], ylim = r0.ylim, main = "$\\mathcal{R}^{({EA})}$: Reasonable Intensity\n Posterior Mode and 90\\% CI",
       xlab = "Day", ylab = "Reproductive Number", type = "l", lwd = MainLineWidth)
  abline(h = seq(0,5,0.2), lty = 3,col="lightgrey")
  abline(h = 1.0, col = "blue", lwd = ThresholdWidth, lty = 2)
  lines(1:(length(r02$mean) - 1), r02$LB[1:(length(r02$LB)-1)], lty = 2, lwd = CI_width)
